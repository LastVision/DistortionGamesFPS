...
namespace Prism
{
	...
	void Camera::OnResize(const int aWidth, const int aHeight)
	{
		...
		mySpecialFoVProjectionMatrix = CU::Matrix44<float>::CreateProjectionMatrixLH(myNear, myFar, static_cast<float>(aHeight) / static_cast<float>(aWidth), myFOV * 0.5f);
	...
	void Camera::Update(float aDeltaTime)
	{
		...
		specialPos -= mySpecialFoVOrientation.GetForward() * 0.5f;
